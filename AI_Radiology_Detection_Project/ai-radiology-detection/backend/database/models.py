...
class Scan(Base):
    ...
    id = Column(get_uuid_type(), primary_key=True, default=get_uuid_default())
    patient_id = Column(get_uuid_type(), ForeignKey('patients.id'))
    uploaded_by = Column(get_uuid_type(), ForeignKey('users.id'))
...
class AIResult(Base):
    ...
    id = Column(get_uuid_type(), primary_key=True, default=get_uuid_default())
    scan_id = Column(get_uuid_type(), ForeignKey('scans.id'))
    model_id = Column(get_uuid_type(), ForeignKey('ai_models.id'))
...
class Report(Base):
    ...
    id = Column(get_uuid_type(), primary_key=True, default=get_uuid_default())
    scan_id = Column(get_uuid_type(), ForeignKey('scans.id'))
    ai_result_id = Column(get_uuid_type(), ForeignKey('ai_results.id'))
    reviewed_by = Column(get_uuid_type(), ForeignKey('users.id'))
...
class AuditLog(Base):
    ...
    id = Column(get_uuid_type(), primary_key=True, default=get_uuid_default())
    user_id = Column(get_uuid_type(), ForeignKey('users.id'))
    action = Column(String(100), nullable=False)
    resource_type = Column(String(50))
    resource_id = Column(get_uuid_type())
    details = Column(JSON)
